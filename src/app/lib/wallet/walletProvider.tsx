...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    wcProvider.on('accountsChanged', onAccounts as unknown as (...args: unknown[]) => void);
    wcProvider.on('chainChanged', onChain as unknown as (...args: unknown[]) => void);
    wcProvider.on('disconnect', onDisconnect);

    return () => {
      wcProvider.removeListener?.('accountsChanged', onAccounts as unknown as (...args: unknown[]) => void);
      wcProvider.removeListener?.('chainChanged', onChain as unknown as (...args: unknown[]) => void);
  ...
  return (
    <WalletContext.Provider
      value={{ connector, provider, signer, address, chainId, connect, disconnect }}
    >
      {children}
    </WalletContext.Provider>
  );
